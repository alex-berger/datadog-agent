...
func (d *directSender) addContainerTags(builder *model.ConnectionBuilder, sourceContainerID *intern.Value, tagsEncoder model.TagEncoder) {
	if sourceContainerID != nil {
		if d.resolver.shouldTagContainer(sourceContainerID) {
			if entityTags, err := d.tagger.Tag(types.NewEntityID(types.ContainerID, getInternedString(sourceContainerID)), types.HighCardinality); err != nil {
				if log.ShouldLog(log.DebugLvl) {
					log.Debugf("error getting tags for container %s: %v", getInternedString(sourceContainerID), err)
				}
